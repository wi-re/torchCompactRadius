...
__global__ void buildNeighborListMLM_Dense_cuda_dispatcher(int32_t numParticles,
    ...
    if(i < numParticles){
        buildNeighborListMLM_Dense<dim, scalar_t>(i, neighborCounters, neighborOffsets, neighborListLength, queryPositions, querySupport, synchronizedSupport, sortedPositions, sortedSupport, minDomain, maxDomain, periodicity, hCell, offsets, cellBegin, cellEnd, cellIndices, cellLevel, cellResolutions,neighborList_i, neighborList_j, buildSymmetric, verbose);
    }
}

template<std::size_t dim = 2, typename scalar_t = float>
__global__ void buildNeighborListMLM_Hashed_cuda_dispatcher(int32_t numParticles,
    cptr_t<int32_t, 1> neighborCounters, cptr_t<int32_t, 1> neighborOffsets, int32_t neighborListLength,
    cptr_t<scalar_t, 2> queryPositions, cptr_t<scalar_t, 1> querySupport, cptr_t<scalar_t, 1> synchronizedSupport,
    cptr_t<scalar_t, 2> sortedPositions, cptr_t<scalar_t, 1> sortedSupport,
    cptr_t<scalar_t, 1> minDomain, cptr_t<scalar_t, 1> maxDomain, cptr_t<bool, 1> periodicity, 
    ...
    int32_t i = blockIdx.x * blockDim.x + threadIdx.x;
    if(i < numParticles){
        buildNeighborListMLM_Hashed<dim, scalar_t>(i, neighborCounters, neighborOffsets, neighborListLength, queryPositions, querySupport, synchronizedSupport, sortedPositions, sortedSupport, minDomain, maxDomain, periodicity, hCell, offsets, cellBegin, cellEnd, cellIndices, cellLevel, cellResolutions, hashMapOffset, hashMapOccupancy, sortedCells, hashMapLength, neighborList_i, neighborList_j, buildSymmetric, verbose);
    }
}
