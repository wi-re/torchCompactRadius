...
auto buildNeighborhoodDense_cpu(int32_t nQuery, int32_t dim, c10::ScalarType scalar, bool isCuda, Ts&&... args) {
    DISPATCH_FUNCTION_DIM_SCALAR(dim, scalar, "countNeighborsMLMParticle", [&]() {
        auto functionArguments = invoke_bool(buildNeighborhoodDense_getFunctionArguments<scalar_t>, isCuda, args...);
        parallelCall(buildNeighborhoodDense_impl<dim_v, scalar_t>, 0, nQuery, functionArguments);
    });
}

std::tuple<torch::Tensor, torch::Tensor> TORCH_EXTENSION_NAME::buildNeighborhoodDense(buildNeighborhoodDense_pyArguments_t) {
    if (verbose)
    ...
    auto wrappedArguments = std::make_tuple(buildNeighborhoodDense_arguments_t);

    if (queryPositions.is_cuda()) {
#ifndef WITH_CUDA
        throw std::runtime_error("CUDA support is not available in this build");
#else
        std::apply(buildNeighborhoodDense_cuda, wrappedArguments);
#endif
    } else {
        buildNeighborhoodDense_cpu(nQuery, dim, queryPositions.scalar_type(), queryPositions.is_cuda(), wrappedArguments);
    }
