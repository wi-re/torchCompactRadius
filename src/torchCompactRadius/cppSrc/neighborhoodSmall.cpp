...
std::pair<torch::Tensor, torch::Tensor> neighborSearchSmall(torch::Tensor queryPositions_, torch::Tensor querySupport_, 
    ...
    auto nNeighbors = neighborOffsetsPtr[nQuery - 1];

    return std::make_pair(neighborCounters, neighborOffsets);

    auto neighborList_i = torch::zeros({nNeighbors}, defaultOptions.dtype(torch::kInt64));
    auto neighborList_j = torch::zeros({nNeighbors}, defaultOptions.dtype(torch::kInt64));
    ...
    at::parallel_for(0, nQuery, 0, [&](int32_t start, int32_t end){
        ...
        neighborSearchSmallCUDA(iPtr, jPtr, neighborOffsetsPtr, queryPositionsPtr, querySupportPtr, referencePositionsPtr, referenceSupportPtr, minDomainPtr, maxDomainPtr, periodicityPtr, nQuery, nReference, dim, searchMode);
    }
