...
// AUTO GENERATE OPTIONAL ACCESSORS
	...
	auto hashMapLength = hashMapLength_.value();
// END AUTO GENERATE OPTIONAL ACCESSORS

                return std::make_tuple(countNeighbors_mlm_argumentsOptional_t);
            }
            else{
            ...
            return std::make_tuple(countNeighbors_mlm_arguments_t);
        }
    }
...
void countNeighborsMLM_cuda(countNeighbors_mlm_functionArgumentsOptional_t);


// the actual implementations
template<std::size_t dim = 2, typename scalar_t = float>
deviceInline auto countNeighborsMLMParticle(int32_t i, countNeighbors_mlm_computeArguments_t){
...
template<std::size_t dim = 2, typename scalar_t = float>
deviceInline auto countNeighborsMLMParticleHashed(int32_t i, countNeighbors_mlm_computeArgumentsOptional_t){
        ...
        neighborHashCollisions[i] = iterateCellHashed<dim>(pos_i, h_i, domainMin, domainMax, periodicity, hCell, offsets, cellBegin, cellEnd, cellIndices, cellLevel, cellResolutions, hashMapOffset, hashMapOccupancy, sortedCells, hashMapLength, [&](int32_t j){
            auto pos_j = sortedPositions[j];
            auto h_j = sortedSupports[j];
            accessCounter++;
            auto dist = modDistance<dim>(pos_i, pos_j, domainMin, domainMax, periodicity);
